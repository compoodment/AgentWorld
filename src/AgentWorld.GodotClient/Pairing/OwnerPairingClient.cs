...
namespace AgentWorld.GodotClient.Pairing;

/// <summary>
/// Default absolute-path endpoints for the owner pairing and signed-action
/// protocol. These paths are relative to the supplied AgentWorld server URI.
/// </summary>
...
public sealed class OwnerPairingClient
{
    ...
    private static Uri BuildEndpointUri(Uri serverBaseUri, string relativeAbsolutePath)
    {
        ...
        {
            throw new ArgumentException("The AgentWorld server URI must be an absolute HTTP(S) URI.", nameof(serverBaseUri));
        }
