...
public static class PhaseFourWorldRuntimeCodec
{
    private const string Header = "agentworld.phase4-runtime/v1";
    ...
    public static byte[] Encode(PhaseFourWorldRuntimeState state)
    {
        Validate(state);
        return JsonSerializer.SerializeToUtf8Bytes(new RuntimeDocument(Header, state), Options);
    }

    public static PhaseFourWorldRuntimeState Decode(ReadOnlyMemory<byte> bytes)
    {
    ...
    public static string Digest(PhaseFourWorldRuntimeState state) =>
        Convert.ToHexStringLower(SHA256.HashData(Encode(state)));

    private static void Validate(PhaseFourWorldRuntimeState state)
    {
        ArgumentNullException.ThrowIfNull(state);
        if (state.SchemaVersion != PhaseFourWorldRuntime.StateSchemaVersion)
        {
    ...
    private sealed record RuntimeDocument(string Format, PhaseFourWorldRuntimeState State);
}
