...
| 3 | Playtest a six-minute day and custom 40-day/four-season year, with at most six hours of life from birth | Fresh host-created worlds now save 360 ticks/day, 40 days/year and four 10-day seasons; day-based stages and a hard 60-day lifespan use the same saved pace. The host still aims for one tick per real second, subject to load. The live VPS now has a fresh paused world with this calendar; its former development save is archived. A completed founder flow and real playtest remain. |
| 4 | Each agent owns a personal model and key; Jev is an optional per-world support layer | The selected agent card and World Settings can choose one provider/model and a named saved key for an inhabitant, applying to both routine and planning decisions. Several keys from the same provider can coexist in private host storage. The per-world Jev switch is saved and functional. The in-world founder flow is still absent; Jev-assisted memory work remains unbuilt. |
...
and durable. The current live VPS has not been rebuilt with this change until
the matching commit passes CI and the service rollout completes.
