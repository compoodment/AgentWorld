...
| --- | --- | --- | --- |
| World host and persistence | **Playable** | Persistent private save, event stream, restart recovery, manual pause and client-presence gate. The paired client can create unlimited named checkpoints of a paused world and load manual or rotating autosaves from the Main Menu; loading keeps the world paused and saves the replaced state first. World Settings controls 1/2/5/10/15/30-minute autosave intervals and off/3/5/10 rotating copies (default on, five minutes, five copies). Per-agent model/credential-slot choices travel with checkpoints, while keys remain in installation storage | One active development world only; no generated-world picker or save migration UI. The per-change active recovery checkpoint remains automatic even when rotating autosaves are off |
...
| Map and movement | **Playable** | Seeded tile map, deterministic routing, animated client movement, wheel zoom, WASD/arrow and middle-drag panning, and a top-left data-drawn overview with click/drag camera navigation. Mountain tiles reject building placement. Terrain draws only camera-visible tiles and the overview uses a compact data atlas, without a per-tile Control node. A generated Small/Medium map can now start, advance and restore through a simulation-only bridge with river/lake/ocean and no-build mountain/peak ground. Generated-map owner observations use a compact row-major terrain payload that the Godot client can draw | The host and normal client still open the fixed 6×5 world; generated worlds are not selectable or playable from the Main Menu. The physical map collapses the generator's climate/surface/vegetation layers; owner snapshots still resend the entire terrain payload, with no chunk streaming. Large/Huge/Mega have compact geography only |
...
| Client presentation | **Playable** | Startup Main Menu with Continue, Load Save, Settings, host pairing and Quit Game; Pause Menu with Save World; shared stores, project phases/blockers, social notes, provider activity and compact per-inhabitant settings | New World and multiple-world selection are unavailable until generated geography replaces the fixed 6×5 camp. Prototype visuals; no dedicated economy/project management screen |
...
| 2 | New World creates a map and empty base camp; four configured founders are added in-world, then Start World begins time | The active live world is now a fresh paused camp with no founders. The paired client chooses each founder's model/key and clicks an empty tile; setup saves after each placement. Start World requires all four and explicit owner action. A separate seeded geography generator has the five named size presets, 64×64 chunk alignment, optional east/west wrap, ocean/lake separation and connected river drainage. A Small/Medium generated map can now be projected into the current physical runtime with an empty camp, four founders, tick advance and save/reload, **but this is not exposed through the host or client**. The live map remains 6×5; climate controls, world preview and multiple-world selection remain unbuilt. |
...
| 5 | One continuous zoomable pixel-art world view with an always-available draggable overview and inspection controls | The Godot build opens at a Main Menu and offers Continue, Settings and pairing. New World and Load World are disabled with an explanation until connected. The world view has single-view zoom, camera-bounded terrain drawing, a compact draggable overview, located event-log navigation, out-of-view event notices, private-thought history and a family tree. Large playable maps, filters and rich belief/compaction memory remain unbuilt. Art and UI are prototype-quality. |
