...
| Map and movement | **Playable** | Seeded tile map, deterministic routing, animated client movement, wheel zoom, WASD/arrow and middle-drag panning, and a top-left data-drawn overview with click/drag camera navigation. Mountain tiles reject building placement | Playable map remains fixed at 6×5 and has no separate peak terrain yet; all tiles are drawn each refresh, without chunk loading or a large-world atlas. A separate deterministic geography generator is verified but not connected to play |
