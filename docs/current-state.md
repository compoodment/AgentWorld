...
Save schema 15 preserves the per-world Jev switch; bounded private thoughts from accepted personal-model decisions, deceased records, optional bounded directed trust and work practice alongside biological
...
| Cognition | **Playable** | World defaults and per-inhabitant provider/model overrides; a saved per-world, pause-only Jev switch that sends Jev-routed routine work to a personal/world planning model or local fallback when off; validation, fallback, retry, safe logs and selection-card telemetry | The two-role provider UI and shared credential slots still do not implement the decided one-personal-model-per-agent flow. Jev has no memory compaction/retrieval role yet; legal planning covers projects, material help, barter and bounded building proposals, not free-form social reasoning |
...
| Family, aging and death | **Integrated but thin** | Separate parental consent, preparation, dependent infants and actual caregiver food/warmth delivery; infants have no paid cognition. Household adults can volunteer to replace lost carers; older dependents accept/refuse independently. Fresh host-created worlds start paused and use six-minute days, a 40-day year, and day-based stages: child at day 3, adult at day 15, elder at day 45, death by day 60 from birth. The card shows age in days. The old development save retains its old saved calendar unless replaced. A pause-only prototype aging override remains under Developer tools | New World still spawns four hard-coded founders and lacks the decided in-world setup gate. Stage-specific social capability and lifespan balance need playtesting; legal guardianship and inheritance remain thin |
...
| 2 | New World creates a map and empty base camp; four configured founders are added in-world, then Start World begins time | Fresh worlds now start paused, but still contain four hard-coded founders and an active settlement. There is no finished new-world setup, multiple-world selection, or four-founder start gate. |
| 3 | Playtest a six-minute day and custom 40-day/four-season year, with at most six hours of life from birth | Fresh host-created worlds now save 360 ticks/day, 40 days/year and four 10-day seasons; day-based stages and a hard 60-day lifespan use the same saved pace. The host still aims for one tick per real second, subject to load. The currently preserved development world has 1,440 ticks/day and 365 days/year and has **not** been reset. A clean new-world/player setup and a real playtest remain. |
| 4 | Each agent owns a personal model and key; Jev is an optional per-world support layer | The existing world can turn Jev routine assistance on/off while paused. Off routes former Jev work to a configured personal/world model or safe local fallback without deleting memories or keys. This is a real saved-world switch, but Jev-assisted memory work, distinct credential identities and the finished one-model-per-agent assignment flow remain to be built. |
