...
| Map and movement | **Playable** | Deterministic Small/Medium generated maps with an empty base camp, rivers/lakes/ocean, no-build mountain/peak terrain, four-founder setup, camera-visible terrain drawing and a draggable top-left overview. Climate is saved separately from provisional sand/forest/snow ground appearances. The older 6×5 world remains selectable. Generated terrain travels in compact row-major owner observations | Surface and vegetation remain collapsed into a few ground kinds; biome/object catalogues and art are unfinished. Whole packed maps still resend every observation and saves still contain per-tile JSON. East/west wrap does not yet apply to all agent routes. Large/Huge/Mega exist only in compact geography generation |
...
| Ecology and weather | **Integrated but thin** | Renewable resources and seasons persist. Generated worlds have deterministic 32×32-tile weather regions weighted by saved local climate; weather affects agents' local warmth/clothing/fire choices, travel fatigue and crop yields at the field. Recent rain and dry days produce a bounded three-day local soil-moisture estimate that can modestly improve or reduce food-crop yields. The top bar follows camera-local weather; World Info also shows nearby soil moisture | Soil moisture is a short derived estimate, not per-tile hydrology. Rich biome weighting, moving weather fronts and precipitation art remain unfinished. Broader ecosystems and balance also need work |
