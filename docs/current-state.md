...
| --- | --- | --- | --- |
| World host and persistence | **Playable** | Persistent private save, event stream, restart recovery, manual pause and client-presence gate. The paired client can create unlimited named checkpoints of a paused world and load one from the Main Menu; loading keeps the world paused and saves the replaced state first. Per-agent model/credential-slot choices travel with checkpoints, while keys remain in installation storage | One active development world only; no generated-world picker, autosave interval/rotation controls, or save migration UI yet |
