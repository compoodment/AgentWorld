...
| Client presentation | **Playable** | Startup Main Menu with Continue, New World, Load World, Settings, pairing and Quit Game; New World creates a paused camp, then founder setup and Start World happen in-world. The pause menu owns Save World/Load Save. In-world panels cover shared stores, work, social notes and agent settings | New World lacks climate controls and an inspectable preview/reroll map; only seed reroll exists. Prototype visuals and no dedicated economy/project management screen |
...
| 2 | New World creates a map and empty base camp; four configured founders are added in-world, then Start World begins time | The signed host and Main Menu now create and select separately saved Small/Medium worlds. Generated worlds open paused at an empty camp; founder placement and explicit Start World are shared with the previous development world. The five named size presets and 64×64 geography foundation exist, but climate controls, map preview and scalable physical-map persistence are still missing; Large/Huge/Mega cannot be played yet. |
