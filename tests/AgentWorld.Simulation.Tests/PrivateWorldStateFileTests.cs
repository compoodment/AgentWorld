...
public sealed class PrivateWorldStateFileTests
{
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"agentworld-history-soak-{Guid.NewGuid():N}");
        try
        {
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"agentworld-history-legacy-{Guid.NewGuid():N}");
        try
        {
    ...
    {
        var directory = Path.Combine(Path.GetTempPath(), $"agentworld-history-{Guid.NewGuid():N}");
        try
        {
            var file = new PrivateWorldStateFile(Path.Combine(directory, "world.json"));
