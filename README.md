...
cognition**. Routine survival choices can use Deterministic or Jev; planning
and work can use Deterministic, OpenAI, or Ollama Cloud. Jev and the selected
large-model provider can therefore remain active at the same time. Hosted
providers choose only among legal intentions generated by the server;
authoritative simulation code continues to validate and execute movement,
survival, work, and world changes.

The host emits low-noise structured events for provider calls, admitted or
fallback intentions, token usage, latency, and client/manual-pause lifecycle
gates. On the packaged systemd service, follow only those events with:

```bash
journalctl -u agentworld-viewer -f -o cat \
  | rg 'cognition_provider_call|cognition_decision|world_tick_gate'
```

These events intentionally exclude keys, authorization data, prompts, raw
provider bodies, and private reasoning.

Provider configuration is stored at `saves/provider-configuration.json` in a
separate atomic file from world state and paired-device authority. The packaged
service uses `/var/lib/agentworld-viewer/provider-configuration.json` with a
`0077` service umask, and the application enforces `0600` file permissions.
The API key is sent only through the signed paired-owner HTTPS boundary, is
never returned to the client, and is not stored by the Windows client or in a
world save. Existing environment variables (`TYPESAFE_API_KEY`,
`OPENAI_API_KEY`, and `OLLAMA_API_KEY`) may seed a new provider file for an
operator-managed migration, but player configuration wins after that file
exists. See the
